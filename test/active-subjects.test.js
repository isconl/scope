...
const PEOPLE = [
  { NAME: 'Jane Doe', GROUP: 'tenant-one', ROLE: 'supervisor (text) - day-to-day director' },
  { NAME: 'John Doe', GROUP: 'tenant-one', ROLE: 'CEO - final authority' },
];
...
test('syncFromCareer creates one engagement row per enabled org, resolving the literal "supervisor" role first', async () => {
  ...
  const client = createActiveSubjectsClient({
    ...store,
    getCareerContext: async () => ({ orgs: [{ id: 'tenant-one', name: 'TenantOne', status: 'active', enabled: true }], people: PEOPLE }),
  });
  ...
  assert.equal(rows[0].SOURCE_REF, 'tenant-one');
  assert.equal(rows[0].SUPERVISOR_OR_CONTACT, 'Jane Doe');
});

test('syncFromCareer matches a shorthand GROUP against a longer org id (real data shape: GROUP:"tenant-one" vs org id "tenant-one")', async () => {
  const store = makeStore({ 'circle/people.tsv': PEOPLE });
  const client = createActiveSubjectsClient({
    ...store,
    getCareerContext: async () => ({ orgs: [{ id: 'tenant-one', status: 'prospect', enabled: true }], people: PEOPLE }),
  });
...
test('syncFromCareer falls back to a CEO/final-authority role when no literal supervisor exists', async () => {
  const noSupervisor = [{ NAME: 'John Doe', GROUP: 'tenant-one', ROLE: 'CEO - final authority' }];
  const store = makeStore({ 'circle/people.tsv': noSupervisor });
  const client = createActiveSubjectsClient({
    ...store,
    getCareerContext: async () => ({ orgs: [{ id: 'tenant-one', status: 'active', enabled: true }], people: noSupervisor }),
  });
  await client.syncFromCareer();
  const rows = await client.listSubjects();
  assert.equal(rows[0].SUPERVISOR_OR_CONTACT, 'John Doe');
});
...
test('syncFromCareer is idempotent: a second run updates the existing row instead of duplicating it', async () => {
  ...
  const client = createActiveSubjectsClient({
    ...store,
    getCareerContext: async () => ({ orgs: [{ id: 'tenant-one', status: 'active', enabled: true }], people: PEOPLE }),
  });
  await client.syncFromCareer();
