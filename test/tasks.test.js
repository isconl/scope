...
test('updateTask sets DELIVERABLE without touching Jira', async () => {
  ...
  const r = await client.updateTask({ taskId: 'T1', deliverable: 'work/files/2026-tenant-one/brief.docx' });
  assert.equal(r.task.DELIVERABLE, 'work/files/2026-tenant-one/brief.docx');
...
test('updateTask sets MODULE, blank clears it back to "-"', async () => {
  ...
  const client = createTasksClient(store);
  await client.updateTask({ taskId: 'T1', module: 'tenant-one-tasks/31' });
  assert.equal(store.data['scope/tasks.tsv'][0].MODULE, 'tenant-one-tasks/31');
