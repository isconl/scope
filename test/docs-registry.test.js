...
test('recordGenerated pushes to OneDrive and stores the real webUrl for a General target', async () => {
  ...
  assert.equal(uploadCall.folderPath, 'TENANT_DOC_ROOT/general');
...
test('recordGenerated pushes a project target straight into ventures.tsv\'s own FOLDER path, no root-prefixing (BA26082402)', async () => {
  ...
  const resolveProjectFolder = async (ventureId) => (ventureId === 'aquifer' ? 'TENANT_PORTFOLIO_ROOT/aquifer-content' : null);
  ...
  assert.equal(uploadCall.folderPath, 'TENANT_PORTFOLIO_ROOT/aquifer-content');
});
...
test('recordGenerated pushes an engagement target into its real discovered OneDrive folder (BA26081803)', async () => {
  const store = makeStore();
  let uploadCall = null;
  const uploadFile = async (folderPath, fileName) => { uploadCall = { folderPath, fileName }; return 'https://onedrive.example/tenant-one-doc'; };
  const resolveEngagementFolder = async (orgId) => (orgId === 'tenant-one' ? '2026-tenant-one' : orgId);
  const client = createDocsRegistryClient({ ...store, uploadFile, resolveEngagementFolder });
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'gd-test-'));
  const written = fakeWritten(dir, { docx: 'doc.docx' });
  const r = await client.recordGenerated({ archetypeId: 'decision-brief', targetKind: 'engagement', targetId: 'tenant-one', written, version: '0.1.0' });
  assert.equal(r.webUrl, 'https://onedrive.example/tenant-one-doc');
  assert.equal(uploadCall.folderPath, 'TENANT_DOC_ROOT/corporate/2026-tenant-one');
});
...
test('recordGenerated falls back to the bare org id for an engagement with no resolvable OneDrive folder', async () => {
  ...
  assert.equal(uploadCall.folderPath, 'TENANT_DOC_ROOT/corporate/hand-added-org');
});
...
test('listDocsMerged (BA26083107): merges a live OneDrive listing for the given engagement, tagged and deduped against generated rows', async () => {
  const store = makeStore({ 'scope/generated_docs.tsv': [
    { ID: 'GD0001', ARCHETYPE_ID: 'a', TARGET_KIND: 'engagement', TARGET_ID: 'tenant-one', FILENAME: 'already-generated.docx', STATUS: 'active', CREATED_AT: '2026-08-19' },
  ] });
  const browseFolder = async (folderPath) => {
    assert.equal(folderPath, 'TENANT_DOC_ROOT/corporate/2026-tenant-one/work-documents');
  ...
  const resolveEngagementFolder = async (id) => (id === 'tenant-one' ? '2026-tenant-one' : id);
  const client = createDocsRegistryClient({ ...store, browseFolder, resolveEngagementFolder });
  const merged = await client.listDocsMerged({}, { id: 'tenant-one', label: 'Tenant One' });
  ...
  assert.equal(onedriveEntry.TARGET_LABEL, 'Tenant One');
...
test('listDocsMerged falls back to generated-only when browseFolder fails, and skips the merge with no mergeEngagement', async () => {
  const store = makeStore({ 'scope/generated_docs.tsv': [
    { ID: 'GD0001', ARCHETYPE_ID: 'a', TARGET_KIND: 'engagement', TARGET_ID: 'tenant-one', FILENAME: 'doc.docx', STATUS: 'active', CREATED_AT: '2026-08-19' },
  ] });
  ...
  const withFailure = await client.listDocsMerged({}, { id: 'tenant-one' });
...
test('listDocsMerged does not merge OneDrive when the filter explicitly scopes to a non-engagement targetKind', async () => {
  ...
  const result = await client.listDocsMerged({ targetKind: 'general' }, { id: 'tenant-one' });
