...
test('jiraListProjects returns key/name/id for each project', () => withFakeHttps(
  () => ({ status: 200, body: { values: [{ key: 'WSRU', name: 'SITE-ALPHA sites Review and Updates', id: '10434' }] } }),
  async () => {
    const client = makeClient();
    const r = await client.jiraListProjects();
    assert.deepEqual(r, [{ key: 'WSRU', name: 'SITE-ALPHA sites Review and Updates', id: '10434' }]);
  },
));
