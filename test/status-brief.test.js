...
const SUBJECT = { SUBJECT_ID: 'SUBJ001', TYPE: 'engagement', SOURCE_REF: 'tenant-one', STATUS: 'active', SUPERVISOR_OR_CONTACT: 'Jane Doe' };
...
test('draftBrief calls spark with the subject/supervisor and stores a draft row', async () => {
  ...
  assert.ok(r.success);
  assert.equal(seenQuery.subjectName, 'tenant-one');
  assert.equal(seenQuery.supervisorName, 'Jane Doe');
