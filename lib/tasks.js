...
function createTasksClient(opts) {
  ...
  function getTask(taskId) {
    return readTSV(tasksFile).find(r => r.ID === taskId) || null;
  }
  ...
  async function createTask(t) {
    ...
    let parentId = '-';
    if (t.parentId) {
      const parent = readTSV(tasksFile).find(r => r.ID === t.parentId);
  ...
  async function updateTask({ taskId, title, priority, status, due_date, tag, start_date, assignee }) {
    ...
    const existing = readTSV(tasksFile).find(r => r.ID === taskId);
    ...
    rewriteTSV(tasksFile, rows => rows.map(r => r.ID === taskId ? updated : r));
  ...
  async function deleteTask({ taskId, deleteJira = true }) {
    if (!taskId) throw new Error('taskId required');
    const target = readTSV(tasksFile).find(r => r.ID === taskId);
    ...
    const removed = rewriteTSV(tasksFile, rows => rows.filter(r => r.ID !== taskId));
