...
function createTasksClient(opts) {
  ...
  async function deleteTask({ taskId, deleteJira = true }) {
    ...
    if (jira && jiraKey && deleteJira) {
      ...
      // FI26091201: a Jira outage should block the delete (nothing else to
      // fall back on), but a 403 (no "Delete Issues" permission) never will
      // and never can succeed on retry -- stranding the local row forever
      // contradicts this module's own stated design (vault is the source
      // of truth; a Jira-side failure is reported, not a transaction
      // failure). Same asymmetry updateTask already gets right.
      if (!jiraResult.success && !jiraResult.permissionDenied) {
        return { failed: true, taskId, jiraKey, jira: jiraResult };
      }
    }

    const removed = await rewriteTSV(tasksFile, rows => rows.filter(r => r.ID !== taskId));
    auditLog.log('task_deleted', { taskId, jiraKey, removedRows: removed, jiraWarning: jiraResult && !jiraResult.success ? jiraResult : null });
