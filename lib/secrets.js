...
function get(name, fallback = '') {
  ...
  // 20 Aug: this exact truthy-check was why this and several other
  // engines' test suites leaked real Bitwarden-sourced secrets into tests
  // that explicitly tried to clear them (the "tasks:" and "jira/preview"
  // test failures here).
...
function applyToEnv() {
  ...
  // get() -- found live 26 Aug 2026, a truthy check here silently clobbered
  // process.env.JIRA_HOST back to the real Bitwarden value before get() ever
  // ran, defeating get()'s own hasOwnProperty fix and causing a test suite
  // to hit the real Jira board (WSRU-106 incident).
