...
function createDocsRegistryClient(opts) {
  const {
    ...
    // KI26091201: these are TENANT-IDENTIFYING OneDrive paths and must never
    // be hardcoded -- this repo is public. They come from the environment,
    // with no default, so a missing value fails loudly at the point of use
    // rather than silently writing to someone else's folder. Set
    // SCOPE_DOC_ROOT_GENERAL / SCOPE_DOC_ROOT_CORPORATE in the deployment
    // environment (Bitwarden -> compose env), never in source.
    generalRoot = process.env.SCOPE_DOC_ROOT_GENERAL || null,
    corporateRoot = process.env.SCOPE_DOC_ROOT_CORPORATE || null,
    // BA26081803: resolves an engagement's real OneDrive folder name
    // (year-prefixed, e.g. `2026-tenant-one` -- the same folder
