...
function createDocsRegistryClient(opts) {
  const {
    ...
    generalRoot = 'TENANT_DOC_ROOT/general',
  } = opts;
  if (!readTSV || !appendTSV || !rewriteTSV) throw new Error('createDocsRegistryClient requires readTSV/appendTSV/rewriteTSV');

  /** BA26081813, General targets only for now. Corporate-engagement
   *  targets stay blocked on PC26081816's org-slug discovery (unchanged).
   *  Project targets are deliberately NOT wired here: the row's own
   *  suggested approach (a `space/data/projects.tsv` ONEDRIVE_PATH column)
   *  doesn't match where Project-target data actually lives -- Writer's
   *  "Project" picker draws from pulse's `finance/ventures.tsv` (personal
   *  finance/venture tracking), not a work-project file, discovered live
   *  20 Aug. Wiring a push against the wrong assumption would silently
   *  write to nonsense paths, so Project stays unbuilt pending a rescope
   *  (see plan.md) rather than guessed through. */
  async function pushToOneDrive({ targetKind, buffer, fileName, contentType }) {
    if (!uploadFile || targetKind !== 'general') return null;
    const folderPath = generalRoot;
  ...
  async function recordGenerated(p) {
    ...
    try {
      ...
      webUrl = await pushToOneDrive({ targetKind: p.targetKind, buffer, fileName: filename, contentType: CONTENT_TYPES[primaryExt] || 'application/octet-stream' });
