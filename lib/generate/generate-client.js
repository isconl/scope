  ...
  async function generate(p) {
    ...
    if (p.write !== false && root) {
      ...
      if (written && docsRegistry) {
        docId = await docsRegistry.recordGenerated({
    ...
    auditLog.log('document_generated', { namespace, archetypeId, formats, written: !!written, docId });
    return { archetype: { id: archetype.id, title: archetype.title }, files, written, docId };
  }
