...
const RENDERERS = {
  ...
  // BA26090601 - fourth, independent output; does not route through or
  // change any of the three renderers above (see render-html.js header).
  html: async (tree) => Buffer.from(renderHtml(tree), 'utf8'),
};
...
function createGenerateClient({ auditLog = { log: () => {} }, docsRegistry = null, outputRoot = null } = {}) {
  ...
  async function generate(p) {
    ...
    if (!formats.length) throw new Error('no valid formats requested (docx, md, pdf, html)');
