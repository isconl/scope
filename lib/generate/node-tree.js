...
/** `details` is an optional, additive, same-length array of per-item
 *  longer text (or null where a given bullet has none) - BA26090601's
 *  html renderer uses it to make a bullet individually expandable
 *  (`<details>/<summary>`) without changing what `items` itself holds.
 *  docx/md/pdf never read `node.details` (they only walk `.items`), so
 *  this is purely additive - existing `bullets(items)` call sites are
 *  untouched and those three renderers need no changes. */
const bullets = (items, details) => {
  const node = { type: 'bullets', items: (items || []).map(String) };
  if (details) node.details = (details || []).map(d => (d == null ? null : String(d)));
  return node;
};
...
/** Label/value pairs, rendered as one line each ("Site.  SITE-BETA, ..."). */
const kvList = (pairs) => ({
  type: 'kv_list',
