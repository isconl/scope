...
function createStyleCorpusClient(opts) {
  ...
  /** Ingests Architect's own user turns from a Claude session transcript (BM26082801).
   *  Turns is an array of { id, text, capturedAt }. Idempotent by SOURCE_ID. */
  async function ingestTurns(turns) {
    if (!Array.isArray(turns) || !turns.length) return { ingested: 0 };
    const existing = await readTSV(corpusFile);
    const alreadyIngested = new Set(existing.map(r => r.SOURCE_ID));
    let ingested = 0;
    const today = new Date().toISOString().slice(0, 10);
    for (const turn of turns) {
      const sourceId = clean(turn.id || turn.uuid || '');
      if (!sourceId || alreadyIngested.has(sourceId)) continue;
      const text = clean(turn.text || turn.content || '');
      if (!text || text === '-') continue;
      await appendTSV(corpusFile, {
        ID: `SC${sourceId.replace(/[^a-zA-Z0-9]/g, '').slice(0, 16)}`,
        SOURCE: 'claude-session',
        PERSON_ID: '-',
        SOURCE_ID: sourceId,
        TEXT: text,
        WORD_COUNT: String((text.match(/\S+/g) || []).length),
        CAPTURED_AT: (turn.capturedAt || turn.timestamp || today).slice(0, 10),
        INGESTED_AT: today,
      });
      ingested += 1;
    }
    auditLog.log('style_corpus_turns_ingested', { ingested, totalTurns: turns.length });
    return { ingested, corpusSize: existing.length + ingested };
  }
  ...
  return { ingestNew, ingestTurns, getStyleProfile, styleStats };
}
