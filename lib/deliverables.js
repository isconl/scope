 ...
 * carry explicit links; they showed up by luck. `TenantOne/work-documents`, where the
...
// KI26091201: the tenant folder name is TENANT-IDENTIFYING and must not be
// hardcoded -- this repo is public. It comes from SCOPE_TENANT_FOLDER in the
// environment. When unset, rootsFor() returns only the engine-local career
// docs: the walk finds nothing tenant-scoped rather than guessing a folder
// name, which is the safe failure direction for a function whose entire
// purpose is to never wander outside declared roots.
const TENANT_FOLDER = process.env.SCOPE_TENANT_FOLDER || null;
...
function rootsFor(workspace, engineDir) {
  const tenantRoots = TENANT_FOLDER ? [
    path.join(workspace, TENANT_FOLDER, 'work-tasks'),
    path.join(workspace, TENANT_FOLDER, 'work_tasks'),      // the folder on disk uses an underscore
    path.join(workspace, TENANT_FOLDER, 'work-documents'),  // the weekly snapshots live here
    path.join(workspace, TENANT_FOLDER, 'work-meetings'),
  ] : [];
  return [
    ...tenantRoots,
