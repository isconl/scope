...
function createStatusBriefClient(opts) {
  ...
  /** FA26091201: interactions.tsv has no ORG_ID -- only PERSON_ID -- so a
   *  subject's org is resolved to its people the same way
   *  active-subjects.js's resolveSupervisor() already does: a GROUP <->
   *  SOURCE_REF substring match, either direction. Without this, every
   *  subject's brief pulled in the whole fleet's last 7 days of touches. */
  async function subjectPersonIds(subject, people) {
    const org = String(subject.SOURCE_REF || '').toLowerCase();
    if (!org) return new Set();
    return new Set(
      people
        .filter(p => {
          const group = String(p.GROUP || '').toLowerCase();
          return group && (org === group || org.includes(group) || group.includes(org));
        })
        .map(p => p.ID)
    );
  }
  ...
  async function gatherActivity(subject) {
    const [tasks, interactions, people] = await Promise.all([
      readTSV(tasksFile).catch(() => []),
      readTSV(interactionsFile).catch(() => []),
      readTSV(peopleFile).catch(() => []),
    ]);
    const personIds = await subjectPersonIds(subject, people);
    ...
    const interactionActivity = interactions
      .filter(i => personIds.has(i.PERSON_ID) && daysAgo(i.DATE) <= 7)
  ...
  /** FA26091202: a malformed cell here used to throw and take out the whole
   *  listing. Degrade that one row to an empty array instead. */
  ...
  async function listBriefs(filter = {}) {
    ...
    return rows.filter(r => !filter.subjectId || r.SUBJECT_ID === filter.subjectId).map(r => ({
      ...r,
      SIGNAL: safeJsonArray(r.SIGNAL),
      SUBSTANCE: safeJsonArray(r.SUBSTANCE),
      TRAJECTORY: safeJsonArray(r.TRAJECTORY),
    }));
  }
  ...
  async function draftBrief(subjectId) {
    ...
    let row;
    if (rewriteTSV) {
      // FA26091202: compute the next ID and append inside the SAME rewrite
      // call, same read-and-write-atomically pattern active-subjects.js
      // already uses -- closes the race two concurrent drafts otherwise hit
      // between reading the max ID and appending.
      await rewriteTSV(briefsFile, (rows) => {
        const n = rows.reduce((m, r2) => Math.max(m, parseInt(String(r2.ID).replace(/\D/g, ''), 10) || 0), 0) + 1;
        row = { ID: `SB${String(n).padStart(4, '0')}`, ...base };
        return [...rows, row];
      });
    } else {
      // No atomic path available -- same race as before, but this is now the
      // deliberately-unsafe fallback, not the only path.
      const rows = await readTSV(briefsFile);
      const n = rows.reduce((m, r2) => Math.max(m, parseInt(String(r2.ID).replace(/\D/g, ''), 10) || 0), 0) + 1;
      row = { ID: `SB${String(n).padStart(4, '0')}`, ...base };
