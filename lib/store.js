 ...
 * directly and only worked because scope and vault happened to share one
...
function createStore({ baseUrl, getToken = () => '', requestImpl, auditLog = { log: () => {} } }) {
  ...
  async function read(collection) {
    ...
    if (r.status !== 200) {
      auditLog.log('scope_vault_read_failed', { collection, status: r.status });
  ...
  async function append(collection, row) {
    ...
    if (r.status !== 200) {
      auditLog.log('scope_vault_append_failed', { collection, status: r.status });
  ...
  async function rewrite(collection, fn) {
    ...
    if (r.status !== 200) {
      auditLog.log('scope_vault_rewrite_failed', { collection, status: r.status });
      throw new Error(`vault rewrite ${collection} failed: HTTP ${r.status}`);
    }
    return r.data.removed;
  }

  /** Upload a Buffer to OneDrive via vault's /onedrive/upload route
   *  (binary content, base64-over-the-wire) -- BA26081813, mirrors
   *  circle/lib/store.js's identical method (same vault route, same
   *  contract, both engines needed it independently). */
  async function uploadFile(folderPath, fileName, buffer, contentType) {
    const r = await request('POST', '/onedrive/upload', {
      folderPath, fileName, contentBase64: buffer.toString('base64'), contentType,
    });
    if (r.status !== 200 || !r.data.ok) {
      auditLog.log('scope_vault_upload_failed', { folderPath, fileName, status: r.status });
      return null;
    }
    // vault's route wraps onedrive-browse.js's upload() result directly --
    // the live Office-Online link lives at .item.webUrl, not top-level
    // (checked live 20 Aug rather than assumed, after circle's own
    // uploadFile() turned out to return a constructed path string instead
    // of vault's real webUrl -- fine for circle's "was this filed" use,
    // wrong for this one, which needs the actual live link).
    return (r.data.item && r.data.item.webUrl) || null;
  }

  return { read, append, rewrite, uploadFile };
}

module.exports = { createStore, defaultRequest };
